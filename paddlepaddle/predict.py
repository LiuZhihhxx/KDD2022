...
def forecast_one(experiment, test_turbines, train_data):
    ...
    model_dir = '{}_t{}_i{}_o{}_ls{}_train{}_val{}_bs{}_lr{}'.format(
        args["filename"], args["task"], args["input_len"], args["output_len"], args["lstm_layer"],
        args["train_size"], args["val_size"], args["batch_size"], args["lr"]
    )
...
def forecast(settings):
    ...
    for i in settings["turbs"]:  # modified 2022/6/25 range(settings["capacity"])
    ...
    for i in range(len(predictions)):
        predictions[i][np.isnan(predictions[i])] = np.random.random()
