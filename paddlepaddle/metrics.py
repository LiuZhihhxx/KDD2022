...
def is_zero_prediction(predictions, identifier, settings):
    ...
    wind_farm_statuses = []
    idx = 0
    for i in settings["turbs"]:  # range(settings["capacity"])
    ...
    non_zero_ratio = non_zero_predictions.size / len(settings["turbs"])  # settings["capacity"]
...
def is_identical_prediction(predictions, identifier, settings):
    ...
    """
    farm_check_statuses = []
    for i in settings["turbs"]:# 改完这里的capacity 然后重新压缩 再evaluation
        idx = 0
        prediction = predictions[idx]
...
def regressor_detailed_scores(predictions, gts, raw_df_lst, settings):
    ...
    for i in range(settings["capacity"]):
        ...
        if not is_valid_prediction(prediction, idx=identifier):
            continue
        gt = gts[settings['turbs'][i]]
        raw_df = raw_df_lst[settings['turbs'][i]]
        # gt = gts[i]
        # raw_df = raw_df_lst[i]
